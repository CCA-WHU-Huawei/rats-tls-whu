...
enclave_attester_err_t enclave_attester_register(const enclave_attester_opts_t *opts)
{
	...
	if (opts->flags & ENCLAVE_ATTESTER_OPTS_FLAGS_CCA_GUEST) {
		if (!is_ccaguest_supported()) {
			// clang-format off
			RTLS_DEBUG("failed to register the attester '%s' due to lack of CCA Guest capability\n",
				   opts->type);
			// clang-format on
			return -ENCLAVE_ATTESTER_ERR_CPU_UNSUPPORTED;
		}
